...
class GaussianDiffusion(nn.Module):
    ...
    def training_losses(self, model, x_start, reweight=False):
        ...
        terms["loss"] = weight.to(th.device('cuda')) * loss.to(th.device('cuda'))
        ...
        terms["loss"] /= pt.to(th.device('cuda'))
        return terms
    ...
    def q_sample(self, x_start, t, noise=None):
        x_start, t=x_start.to(th.device('cuda')), t.to(th.device('cuda'))
        if noise is None:
            noise = th.randn_like(x_start).to(th.device('cuda'))
    ...
    def q_posterior_mean_variance(self, x_start, x_t, t):
        ...
        Compute the mean and variance of the diffusion posterior:
            q(x_{t-1} | x_t, x_0)
        """
        x_start, x_t, t=x_start.to(th.device('cuda')),x_t.to(th.device('cuda')),t.to(th.device('cuda'))
